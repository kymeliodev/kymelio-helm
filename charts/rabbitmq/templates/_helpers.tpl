...
{{/*
Name of the Secret holding the broker credentials.
*/}}
...
{{/*
Resolve the default user password. Reuse an existing value on upgrade so a
...
{{- else }}
{{- $existing := lookup "v1" "Secret" .Release.Namespace (include "rabbitmq.fullname" .) }}
...
{{/*
Resolve the Erlang cookie. Reuse an existing value on upgrade so a generated
cookie stays stable across releases.
*/}}
...
{{- else }}
{{- $existing := lookup "v1" "Secret" .Release.Namespace (include "rabbitmq.fullname" .) }}
{{- if and $existing (index $existing.data .Values.auth.secretKeys.erlangCookieKey) }}
{{- index $existing.data .Values.auth.secretKeys.erlangCookieKey | b64dec }}
{{- else }}
{{- randAlphaNum 32 }}
{{- end }}
{{- end }}
