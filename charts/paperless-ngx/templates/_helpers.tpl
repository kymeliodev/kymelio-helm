...
app.kubernetes.io/part-of: {{ include "paperless-ngx.name" . }}
{{- end }}
...
{{/*
Resolve the secret key. Reuse the value stored in an existing release Secret
so the generated value stays stable across upgrades.
*/}}
{{- define "paperless-ngx.secretKey" -}}
{{- if .Values.auth.secretKey }}
{{- .Values.auth.secretKey }}
{{- else }}
{{- $existing := lookup "v1" "Secret" .Release.Namespace (include "paperless-ngx.fullname" .) }}
